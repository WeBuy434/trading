...
class CryptoNotification extends MySQL {
   ...
   */
  public function __construct($symbol = null, $user = null, $notificationid = null){
  ...
  public function _generateTitleNotification($Coin, $value){
    return $Coin->_getSymbol().' '.($this->_isCompareUnder() ? 'decreased' : 'increased').' to '.number_format($value, 2, ',', ' ').' '.$Coin->_getApi()->_getCurrency();
  }
  ...
  public function _getListCryptoNotifications(){
    $listNotifiation = [];
    foreach (parent::querySqlRequest("SELECT * FROM notification_krypto WHERE symbol_notification=:symbol_notification AND status_notification=:status_notification AND id_user=:id_user ORDER BY id_notification DESC",
                                      [
   ...
   */
  public function _createNotification($value, $actual){
    ...
    $r = parent::execSqlRequest("INSERT INTO notification_krypto (symbol_notification, actual_value_notification, value_notification, compare_notififcation, id_user)
                                VALUES (:symbol_notification, :actual_value_notification, :value_notification, :compare_notififcation, :id_user)",
                                [
                                  'symbol_notification' => $this->_getSymbol(),
