...
class AppModule {
  ...
  public function _loadAssets($type = "css"){
    ...
    foreach (scandir($this->_getModulePath().'/statics/'.$type) as $asset) {
      ...
      if($type == "css"){
        $res[] = '<link rel="stylesheet" href="'.$this->_getModuleURL().'/statics/'.$type.'/'.$asset.'">'; // Load css stylesheet
      }
      if($type == "js"){ // If assets need is JS
        $res[] = '<script src="'.$this->_getModuleURL().'/statics/'.$type.'/'.$asset.'" charset="utf-8"></script>'; // Load JS script
      }
    }
    return $res;
  }
