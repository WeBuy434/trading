...
class User extends MySQL {
  ...
  public function _getUserSettingsKey($key){
    if(!array_key_exists($key, $this->UserSettings) && !array_key_exists($key, $this->_getDefaultUserSettings())) throw new Exception("Error : User data not exist for key = ".$key, 1);
  ...
  public function _getLang(){
    if(!empty($_SESSION['kr_custom_lang'])) return $_SESSION['kr_custom_lang'];
  ...
  public function _setLanguage($lang){
    if(!Lang::languageAvailable($lang)) throw new Exception("Language not available", 1);
  ...
  public function _saveUserLoginHistory(){
    ...
    // Get geoip location
    $ch =  curl_init('http://geoip.nekudo.com/api/'.$CurrentUserIP);
    ...
    $geoAdd = true;
    if(array_key_exists('type', $s) && $s['type'] == "error"){
    ...
    $r = parent::execSqlRequest("INSERT INTO user_login_history_krypto (id_user, date_user_login_history, ip_user_login_history, device_user_login_history, location_user_login_history, country_code_user_login_history)
                                ...
                                [
                                  ...
                                  'location_user_login_history' => ($geoAdd ? ($s['city'] != 'false' ? $s['city'].' ('.$s['country']['name'].')' : $s['country']['name']) : ''),
                                  'country_code_user_login_history' => ($geoAdd ? $s['country']['code'] : '')
                                ]);
      ...
      if($ipUnknow){
        ...
        $App->_sendMail($this->_getEmail(), $App->_getAppTitle().' - Successful Login From New IP '.$CurrentUserIP.' - '.date('d/m/Y H:i:s', time()), $template->render([
          ...
          'IP' => $CurrentUserIP,
          'LOCATION' => ($geoAdd ? ($s['city'] != 'false' ? $s['city'].' ('.$s['country']['name'].')' : $s['country']['name']) : 'Unknow'),
