...
<section class="kr-account-config-exchange">
  ...
  foreach ($Trade->_getThirdParty() as $key => $Exchange) {
    ?>
    <div onclick="_showThirdpartySetup('<?php echo $Exchange->_getExchangeName(); ?>');" class="<?php if($Exchange->_isActivated() != false) echo 'kr-account-config-exchange-enable'; ?>">
      <div>
        <svg class="lnr lnr-checkmark-circle"><use xlink:href="#lnr-checkmark-circle"></use></svg>
      </div>
      <img src="<?php APP_URL; ?>/assets/img/icons/trade/<?php echo $Exchange->_getLogo(); ?>" alt="<?php echo $Exchange->_getName(); ?>">
    </div>
    <?php
  }
  ?>
