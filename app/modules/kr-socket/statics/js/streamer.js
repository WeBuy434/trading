 ...
 */
function addSubscribtion(symbol, currency, type = 5){

	if(jQuery.inArray(type + '~CCCAGG~' + symbol + '~' + currency, currentSub) != -1) return false;
	...
	if(type == "0"){
		...
		currentSub.push(type + '~CCCAGG~' + symbol + '~' + currency);
	  socket.emit('SubAdd', { subs: [type + '~CCCAGG~' + symbol + '~' + currency] });
	}
}
 ...
 * @param  {Number} [type=5] Type subscription
 */
function deleteSubscription(symbol, currency, type = 5){
  socket.emit('SubRemove', { subs: [type + '~CCCAGG~' + symbol + '~' + currency] });
	currentSub	= $.grep(currentSub, function(value) {
	  return value != type + '~CCCAGG~' + symbol + '~' + currency;
	});
}
