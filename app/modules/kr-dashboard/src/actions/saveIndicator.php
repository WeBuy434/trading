...
try {
    ...
    $indicatorContainer = $SqlRequest->querySqlRequest("SELECT * FROM indicators_krypto WHERE key_graph=:key_graph AND symbol_indicators=:symbol_indicators
                                                                        AND key_indicators=:key_indicators",
                                                      [
    ...
    if (count($indicatorContainer) > 0) {
        ...
        $r = $SqlRequest->execSqlRequest("UPDATE indicators_krypto SET data_indicators=:data_indicators, title_indicators=:title_indicators
        ...
        if (!$r) {
            throw new Exception("Error : Fail to update indicators", 1);
        }
    } else {
        // Insert indicator
        $r = $SqlRequest->execSqlRequest("INSERT INTO indicators_krypto (key_graph, key_indicators, symbol_indicators, data_indicators, title_indicators)
        ...
        if (!$r) {
            throw new Exception("Error : Fail to insert indicators", 1);
        }
    }
