 ...
 */
function changeGraph(dataCoin, currency, container, topitem, replace = false){
  ...
  loadChart(dataCoin.symbol, null, container);
  ...
  if(replace){
    ...
    $('.kr-top-graphlist-item[topitem="' + topitem + '"]').find('.kr-top-graphlist-inf').find('label').html(dataCoin.symbol);
    $('.kr-top-graphlist-item[topitem="' + topitem + '"]').find('.kr-top-graphlist-pic').html(dataCoin.icon);
    addSubscribtion(dataCoin.icon, currency);
  }

  reloadTopListGraph();

  // Save new chart data
  $.post($('body').attr('hrefapp') + "/app/modules/kr-dashboard/src/actions/changeGraph.php", {container:container, topitem:topitem, coinsymbol:dataCoin.symbol}).done(function(data){
    // Parse json data
...
function removeGraph(container){
  ...
  if(!f){
    ...
    $('#' + container).html('<div class="kr-dash-pan-lgl">' +
      '<div class="kr-dash-pan-cry-select" graph="' + $('#' + container).attr('id') + '" style="display: none;">' +
        ...
        '</ul>' +
      '</div>' +
      '<img src="' + $('body').attr('hrefapp') + '/assets/img/logo.svg" alt="">' +
    '</div>');
 ...
 */
function loadChart(symbol, callback, container){

  reloadTopListGraph();

  // Load chart content
  $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChartContent.php', {coin:symbol, container:container}).done(function(data){
    ...
    try { // Try to parse data in JSON -> true = error
      ...
      $.when($('#' + container).html(data)).then(function(){ // Add graph in container & load data
        ...
        loadChartData(symbol, function(data){
          ...
          chartList[container]['option'] = {
              ...
              // graphic: { //   elements: [ //     { //       type: 'line', //       draggable: true, //       shape: { //         x1: 0, //         y1: 0, //         x2: 500, //         y2: 500 //       }, //       style: { //         fill:'#FF0000' //       } //     } //   ] // },
              tooltip: {
                  trigger: 'axis',
          ...
          $('#graph-' + container).dblclick(function(){
            createNewNotification(symbol, $('#' + container).attr('currency'), currentValueSelected);
          });
          ...
          subscribeStreamerCallback(function(dataCoin){
            ...
            if(dataCoin.FROMSYMBOL == symbol && $('#' + container).attr('symbol') == symbol){
              ...
              if($('#' + container).hasClass('kr-dash-pan-cry-selected') || ($('.kr-dash-pan-cry-selected').length == 0 && $('.kr-dash-pan-cry').index($('#' + container)) == 0)){
                // Change page title
                appendPageTitle(dataCoin.FROMSYMBOL + ': ' + dataCoin.PRICE + ' ' + (parseFloat(dataCoin.CHANGE24HOURPCT) < 0 ? '▼' : '▲') + ' ' + dataCoin.CHANGE24HOURPCT + '%');
              }
 ...
 */
function loadChartData(symbol, fnc = null, container){
  // Get chart data
  $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChart.php', {coin:symbol, type:'load'}).done(function(data){
...
function syncChart(container){
  ...
  // Get graph data
  $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChart.php', {coin:symbol, type:'update'}).done(function(data){
