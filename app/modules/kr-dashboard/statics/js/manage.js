...
function initCoinGraphControllers(){
  ...
  $('.kr-dash-pan-cry').each(function(){
    ...
    $(this).find('.kr-dash-pan-type-s > div').off('click').click(function(){

      $(this).toggleClass('kr-dash-pan-type-sdone');

      let newType = "candlestick";
      if(chartList[container]['option']['series'][0]['itemStyle']['normal']['opacity'] != 0){
      ...
      $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/changeTypeGraph.php', {graph_id:$('#' + container).attr('graph-id')}).done(function(data){
    ...
    $(this).find('.kr-dash-pan-cry-select').find('input[type="text"]').off('keyup').keyup(function(){
      updateListCoinGraph($(this).val(), $(this).attr('graph'), function(coin){
        ...
        addTopListGraph(coin, function(topItem){
          changeGraph(coin, $('.kr-dash-pan-cry.kr-dash-pan-cry-selected').attr('currency'), $('.kr-dash-pan-cry.kr-dash-pan-cry-selected').attr('id'), topItem.attr('topitem'), true);
        });
      });
    });
    ...
    $(this).find('.kr-dash-tgglfullscreen').off('click').click(function(){
      toggleGraphFullScreen('#' + container);
    });

    // Close graph
    $(this).find('.kr-dash-close').off('click').click(function(){
      removeGraph(container);
    });
