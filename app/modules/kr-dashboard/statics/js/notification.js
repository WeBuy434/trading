...
function addChartNotification(chart, symbol, val){

  $.each($('.kr-dash-pan-cry[symbol="' + symbol + '"]'), function(){
    loadChartNotification($(this).attr('id'), symbol, val, null);
  });

  $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/createNotification.php', {symb:symbol, value:val}).done(function(data){
...
function createNewNotification(symbol, currency, val = null){
  $('body').addClass('kr-nblr');
  $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/createAlert.php', {symb:symbol, currency:currency, click:val}).done(function(data){
    try {
  ...
  $('form.createalert-popup-frm').off('submit').submit(function(e){
    ...
    if(allValid){
      for (val of $(this).serializeArray()) {
        if(val.name != "symbol_alert"){
          if(val.value.length > 0){
            addChartNotification(null, lVal[2].value, val.value);
          }
        }
      }
    }
