 ...
 */
function addGraphDashboard(coin, currency){
  ...
  $('.kr-top-graphlist-item').each(function(){
    ...
    $(this).off('click').click(function(){
      ...
      if($('.kr-dash-chart-n').length > 0){
        ...
        changeView('coin', 'coin', {symbol:$(this).attr('symbol')});
      }
    ...
    if($('.kr-dash-pan-cry[container="' + $(this).attr('container') + '"][symbol="' + $(this).attr('symbol') + '"]').length == 0){
 ...
 */
function changeSelectedTopList(container, symbol){
  $('.kr-top-graphlist-item').removeClass('kr-top-graphlist-item-selected');
  $('.kr-top-graphlist-item[container="' + container + '"][symbol="' + symbol + '"]').addClass('kr-top-graphlist-item-selected');
}
...
function addTopListGraph(coin, callback = null){
  ...
  $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/addTopList.php', {symbol:coin.symbol}).done(function(data){

    let response = jQuery.parseJSON(data);
    if(response.error == 0){
      let topListItem = $('<li class="kr-mono kr-top-graphlist-item" container="" topitem="' + response.item_id + '" symbol="' + coin.symbol + '" coinname="' + coin.name + '" currency="' + coin.currency + '" pasth="" kr-val-graph="">' +
                    '<div class="kr-top-graphlist-closeb">' +
                      '<svg class="lnr lnr-cross"><use xlink:href="#lnr-cross"></use></svg>' +
                    '</div>' +
                    '<div class="kr-top-graphlist-pic">' + coin.icon + '</div>' +
                    '<div class="kr-top-graphlist-inf"><label>' + coin.symbol + '</label><span kr-data="CHANGE24HOURPCT">~</span>' +
                    '</div></li>');
...
function changeTopItemAction(item){
  if(!item.hasClass('kr-top-graphlist-item-view')){
    ...
    }, item.attr('currency'), containerID, item.attr('topitem'));


  } else {
    // If selected item is clicked item -> make boucne animation
    if(item.hasClass('kr-top-graphlist-item-selected')){
      let graphContent = $('.kr-dash-pan-cry[container="' + item.attr('container') + '"][symbol="' + item.attr('symbol') + '"]');
