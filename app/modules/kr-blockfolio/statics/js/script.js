  ...
  subscribeStreamerCallback(function(dataCoin){
    if(dataCoin.PRICE != undefined){
      $('.kr-port').find('[symbol="' + dataCoin.FROMSYMBOL + '"][currency="' + dataCoin.TOSYMBOL + '"]').each(function(){
        $(this).find('[kr-port-d="PRICE"]').html(KRformatNumber(dataCoin.PRICE, (dataCoin.PRICE < 2 ? 6 : 2)) + ' ' + dataCoin.TOSYMBOL);
        ...
        $(this).find('[kr-holding-market-value]').html(KRformatNumber(holdingSize * dataCoin.PRICE, 2) + ' ' + currencySymbol);

        let diffProfLoss = (holdingSize * dataCoin.PRICE) - parseFloat($(this).find('.kr-port-holding').attr('kr-holding-buy-value'));
        $(this).find('[kr-holding-profit-loss]').html(KRformatNumber(diffProfLoss, 2) + ' ' + currencySymbol);
...
function addCryptoBlockfolio(symbol){
  showDashboardLoading();
  $.post($('body').attr('hrefapp') + '/app/modules/kr-blockfolio/src/actions/addItem.php', {symbol:symbol}).done(function(data){
